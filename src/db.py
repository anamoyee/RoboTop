...
def _opendb(func):
  ...
  def wrapper(*args, **kwargs):
    with shelve.open(str(args[0]._path)) as _db:
      if "_db" in kwargs: tcr.console.error("Do not pass _db to database calls, it's handled automatically, the value was voided")
...
class _DBInterace(ABC):
  ...
  def keys(self, *, tolist: bool = True, _db: shelve.Shelf) -> KeysView:
    ...
    a = _db.keys()
    if tolist: a = copy.deepcopy(list(a))
    return a
  ...
  def items(self, *, todict: bool = True, _db: shelve.Shelf) -> ItemsView:
    ...
    a = _db.items()
    if todict: a = copy.deepcopy(dict(a))
    return a
  ...
  def read(self, key: int | str, *, _db: shelve.Shelf) -> dict:
    ...
    if "first_seen" in result and result["first_seen"] is None: result["first_seen"] = unix()
  ...
  def write(self, key: int | str, data: dict | None = None, *, _db: shelve.Shelf, **kwargs) -> dict:
    ...
    if "first_seen" in merged and merged["first_seen"] is None: merged["first_seen"] = unix()
    _db[key] = merged
    return _db[key]
  ...
  def fix(self, *, _db: shelve.Shelf) -> dict:
    ...
    for k, v in new_db.copy().items():
      ...
      if "first_seen" in new_db[k] and new_db[k]["first_seen"] is None: new_db[k]["first_seen"] = unix()
